...
// structure to contain all our info so we can pass it to callback
...
int main(int arg, char *argv[]){
  ...
  if(!data.pipeline || !data.source || !data.tee || !data.audio_queue || !data.convert
        || !data.resample || !data.wavenc || !data.sink || !data.wave_queue || !data.visual
        || !data.wave_convert || !data.wave_sink) {
    g_printerr("Not all elements could be created\n");
  }

  //build the pipeline. note that we are not linking the source at this point--will do it later
  gst_bin_add_many(GST_BIN(data.pipeline), data.source, data.tee, data.audio_queue, data.convert,
                   data.resample, data.wavenc, data.sink, data.wave_queue, data.visual, data.wave_convert,
                   data.wave_sink, nullptr);

  if(!gst_element_link_many(data.convert, data.resample, data.tee, nullptr) ||
      !gst_element_link_many(data.audio_queue, data.wavenc, data.sink, nullptr) ||
      !gst_element_link_many(data.wave_queue, data.visual, data.wave_convert, data.wave_sink, nullptr)){
  ...
  g_object_set(data.sink, "location", "test.wav", nullptr);
